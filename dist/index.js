...
export { getUniqueID, openUrl, hasParents, renderRules, AstRenderer, parser, stringToTokens, tokensToAST, MarkdownIt, styles, removeTextStyleProps, FitImage, textStyleProps }; // we use StyleSheet.flatten here to make sure we have an object, in case someone
...
const getStyle = (mergeStyle, style) => {
  ...
  if (mergeStyle === true && style !== null) {
    ...
    Object.keys(style).forEach(value => {
      useStyles[value] = { ...StyleSheet.flatten(style[value])
      };
    }); // combine any existing styles

    Object.keys(styles).forEach(value => {
      useStyles[value] = { ...styles[value],
...
const getRenderer = (renderer, rules, style, mergeStyle, onLinkPress, maxTopLevelChildren, topLevelMaxExceededItem, allowedImageHandlers, defaultImageHandler, debugPrintTree) => {
  ...
  if (renderer) {
    ...
    return new AstRenderer({ ...renderRules,
      ...(rules || {})
